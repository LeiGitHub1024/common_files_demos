# 常见文件类型示例库

本仓库提供了12类、48种常见文件类型的示例和说明，方便开发者快速了解各种文件格式。

## 📁 文件分类

### 1. 文本文件 (text_files)

用于存储纯文本或格式化文本内容的文件类型。

- **.txt** - 纯文本文件，最简单的文本格式
- **.doc / .docx** - Microsoft Word文档格式
- **.pdf** - 便携式文档格式，跨平台通用
- **.rtf** - 富文本格式，支持基本格式化
- **.odt** - OpenDocument文本格式，开源标准

**示例文件**: `text_files/example.txt`

### 2. 图片文件 (image_files)

用于存储图像数据的文件类型。

- **.jpg / .jpeg** - JPEG图像格式，有损压缩，适合照片
- **.png** - PNG图像格式，支持透明背景
- **.gif** - GIF图像格式，支持动画
- **.bmp** - 位图格式，无压缩，文件较大
- **.svg** - 可缩放矢量图形，基于XML
- **.tiff** - TIFF图像格式，支持无损压缩

**注意**: 图片文件为二进制格式，本仓库仅提供空文件夹，请自行添加示例图片。

### 3. 音频文件 (audio_files)

用于存储音频数据的文件类型。

- **.mp3** - MP3音频格式，有损压缩，广泛使用
- **.wav** - WAV音频格式，无损压缩，文件较大
- **.aac** - AAC音频格式，高效压缩
- **.flac** - FLAC音频格式，无损压缩
- **.ogg** - OGG音频格式，开源标准

**注意**: 音频文件为二进制格式，本仓库仅提供空文件夹，请自行添加示例音频。

### 4. 视频文件 (video_files)

用于存储视频数据的文件类型。

- **.mp4** - MP4视频格式，广泛使用的容器格式
- **.avi** - AVI视频格式，Windows常用
- **.mkv** - Matroska视频格式，开源容器
- **.mov** - QuickTime视频格式，Apple标准
- **.wmv** - Windows Media Video格式

**注意**: 视频文件为二进制格式，本仓库仅提供空文件夹，请自行添加示例视频。

### 5. 压缩文件 (archive_files)

用于压缩和打包多个文件的文件类型。

- **.zip** - ZIP压缩格式，最常用的压缩格式
- **.rar** - RAR压缩格式，高压缩比
- **.7z** - 7-Zip压缩格式，开源高效
- **.tar** - TAR归档格式，Unix/Linux常用
- **.gz** - GZIP压缩格式，常与tar配合使用

**注意**: 压缩文件为二进制格式，本仓库仅提供空文件夹，请自行添加示例压缩包。

### 6. 可执行文件 (executable_files)

可直接运行的程序文件。

- **.exe** - Windows可执行文件
- **.bat** - Windows批处理脚本文件
- **.sh** - Unix/Linux Shell脚本文件
- **.msi** - Windows安装程序文件

**示例文件**:

- `executable_files/example.sh` - Shell脚本示例
- `executable_files/example.bat` - 批处理脚本示例

### 7. 表格文件 (spreadsheet_files)

用于存储电子表格数据的文件类型。

- **.xls / .xlsx** - Microsoft Excel表格格式
- **.ods** - OpenDocument电子表格格式

**注意**: 表格文件为二进制格式，本仓库仅提供空文件夹，请自行添加示例表格。

### 8. 演示文稿文件 (presentation_files)

用于存储演示文稿的文件类型。

- **.ppt / .pptx** - Microsoft PowerPoint演示文稿格式
- **.odp** - OpenDocument演示文稿格式

**注意**: 演示文稿文件为二进制格式，本仓库仅提供空文件夹，请自行添加示例演示文稿。

### 9. 数据库文件 (database_files)

用于存储数据库结构和数据的文件类型。

- **.mdb / .accdb** - Microsoft Access数据库文件
- **.sql** - SQL脚本文件，包含数据库操作语句
- **.db** - SQLite数据库文件

**示例文件**: `database_files/example.sql` - SQL脚本示例

### 10. 网页文件 (web_files)

用于构建网页的文件类型。

- **.html / .htm** - HTML标记语言文件
- **.css** - 层叠样式表文件
- **.js** - JavaScript脚本文件

**示例文件**:

- `web_files/example.html` - HTML示例
- `web_files/example.css` - CSS样式示例
- `web_files/example.js` - JavaScript示例

### 11. 代码文件 (code_files)

用于存储程序源代码的文件类型。

- **.c / .cpp** - C/C++源代码文件
- **.java** - Java源代码文件
- **.py** - Python源代码文件
- **.php** - PHP源代码文件

**示例文件**:

- `code_files/example.c` - C语言示例
- `code_files/example.cpp` - C++示例
- `code_files/example.java` - Java示例
- `code_files/example.py` - Python示例
- `code_files/example.php` - PHP示例

### 12. 配置文件 (config_files)

用于存储应用程序配置信息的文件类型。

- **.ini** - INI配置文件格式
- **.json** - JSON配置文件格式
- **.xml** - XML配置文件格式
- **.yaml** - YAML配置文件格式

**示例文件**:

- `config_files/example.ini` - INI配置示例
- `config_files/example.json` - JSON配置示例
- `config_files/example.xml` - XML配置示例
- `config_files/example.yaml` - YAML配置示例

## 📊 统计信息

- **文件类别**: 12类
- **文件格式**: 48种
- **示例文件**: 已提供文本格式文件的示例

## 🚀 使用方法

1. 浏览各个文件夹了解不同文件类型
2. 查看示例文件了解文件格式和结构
3. 根据需要添加自己的示例文件

## 📝 注意事项

- 二进制文件（图片、音频、视频、压缩包、Office文档等）无法直接以文本形式展示，因此仅提供空文件夹
- 所有文本格式的文件都已提供示例
- 欢迎补充更多示例文件

## 🤝 贡献

欢迎提交更多文件类型的示例，帮助完善这个示例库！
